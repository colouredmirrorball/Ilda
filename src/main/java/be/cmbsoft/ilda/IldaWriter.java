...
public class IldaWriter {
     ...
     * @param location    The path to where the ilda file should be exported
     * @param frames      All frames that should be included in the ilda file
     * @param palette     An IldaPalette that will be appended in front of the ilda file with a
     *                    format 2 header
     * @param ildaVersion ilda format: should be 0 or 1 since only those two formats use a
     *                    palette for their colour information
     *                    but nobody is stopping you from appending a palette to a format 4/5
     *                    file, though that would be pointless
     */
    ...
    public static void writeFile(String location, IldaFrame[] frames, IldaPalette palette,
        int ildaVersion) {
        if (frames == null) {return;}
        IldaFrame.fixHeaders(frames);
    ...
    /**
     * This method returns a byte array which can be exported directly as an ilda file from a
     * palette and an ArrayList of IldaFrames.
     ...
     * @param ildaVersion the ilda format the frames get saved as, can be 0, 1, 4 or 5 but only 0
     *                   and 1 use a palette. It makes no sense to export as format 4 or 5 with a
     *                   palette included.
    ...
    public static byte[] getBytesFromFrames(IldaFrame[] frames, IldaPalette palette,
        int ildaVersion) {
    ...
    /**
     * This method returns a byte array from only an ArrayList of IldaFrames. This array can be
     * saved to disk directly as a valid ilda file (binary file).
     *
    ...
    public static byte[] getBytesFromFrames(IldaFrame[] frames, int ildaVersion) {
        ...
        if (frames.length == 0) {return new byte[0];}

        for (IldaFrame frame : frames) {
            writeCommonIldaHeader(theBytes);

            if (ildaVersion == 0 || ildaVersion == 1 || ildaVersion == 2 || ildaVersion == 4 ||
                ildaVersion == 5) {theBytes.add((byte) ildaVersion);} else {
                return new byte[0];
            }
            ...
            {
                char letter;
                if (frame.frameName.length() < i + 1) {letter = ' ';} else {
                    letter = frame.frameName.charAt(i);
                }
            ...
            if (frame.companyName.length() == 0)   //Bytes 17-24: Company Name
            {
                ...
                for (int i = 0; i < 8; i++) {
                    char letter;
                    if (frame.companyName.length() < i + 1) {letter = ' ';} else {
                        letter = frame.companyName.charAt(i);
                    }
                    theBytes.add((byte) letter);
                }
            }
            ...
            for (IldaPoint point : frame.points) {
                ...
                if (ildaVersion == 0 || ildaVersion == 1) {theBytes.add((point.palIndex));} else {
                    int c = point.colour;

                    if (point.blanked) {
                        c = 0;  //some programs only use colour information to determine blanking
                    }
