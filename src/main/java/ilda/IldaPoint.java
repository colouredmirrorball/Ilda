...
/**
 * A point of an ilda frame. Position is stored in a PVector with bounds [-1..1]. Colour is
 * stored in an integer which
 * is a 32-bit number: the first eight bits are not used, the second eight bits are red (0-255),
 * the next eight
 * represent green and the last eight bits are blue. This is the "official" colour
 * representation: points also store a
 * palIndex but this is only used to set the colour of a palette, never to render it to a screen.
 * A point also has a
...
public class IldaPoint {
    ...
    /**
     * This method picks the best fitting palette colour that matches this point's RGB value. The
     * palette index of this
    ...
    /**
     * The position should be normalised so that the fields x, y and z of the argument PVector
     * are always in the
     * interval -1..1
     *
    ...
    /**
     * Set the blanked flag of a point Blanked means the lasers will not turn on at this point
     * but the scanners will
